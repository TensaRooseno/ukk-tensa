...
<head>
    ...
    <style>
        .sidebar {
            ...
            width: 200px;
            padding-top: 20px;
            background-color: #28a745; /* Green theme for cashier */
        }
        .sidebar .nav-link {
            color: #fff;
        }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#addItem').click(function() {
                ...
                // Check if the product is already in the transactionItems array
                ...
                if (existingItemIndex !== -1) {
                    ...
                    // If the product does not exist, fetch and add it
            ...
            $('#processTransaction').click(function() {
                ...
                $.post('transaction.php', { action: 'process', items: transactionItems, discount, amount_paid: amountPaid }, function(response) {
                    const data = JSON.parse(response);
                    if (data.error) {
                        alert(data.error);
                    } else {
            ...
            $(document).on('input', '#discount, #amountPaid', function() {
                ...
                $('#changeGiven').text(changeGiven >= 0 ? `$${changeGiven.toFixed(2)}` : 'Insufficient amount');
            });
