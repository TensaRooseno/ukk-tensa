...
// Get transaction data - ensure cashier can only access their own transactions
$query = "SELECT t.*, u.username, m.phone_number as member_phone 
          FROM transactions t 
          JOIN users u ON t.cashier_id = u.user_id 
          LEFT JOIN members m ON t.member_id = m.id
          WHERE t.transaction_id = $transaction_id AND t.cashier_id = $user_id";
$result = mysqli_query($conn, $query);

if (!$result || mysqli_num_rows($result) === 0) {
    die("Transaction not found or you don't have permission to access it");
}
